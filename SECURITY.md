...
| **Web UI localhost-only** | The combined setup wizard + dashboard (127.0.0.1:8721) binds localhost exclusively — nothing is exposed to the network; remote access requires an SSH tunnel. The dashboard chat is a fixed command allowlist plus answer-only LLM Q&A; it cannot place trades. |
| **Self-edit — human merge gate** | Off by default (`self-edit on` to enable). The agent's LLM can WRITE changes to its own code, but nothing applies until the owner reviews the unified diff in the dashboard and clicks Apply; applies are backed up, compile-checked, and auto-rolled-back on failure; and a **FOUNDATION layer is immutable to the agent**: agent.py (consent gate + safety pipeline), self_edit.py (the governor itself), llm_policy.py, DISCLAIMER.md, SECURITY.md, install.sh, LICENSE can never be self-rewritten — enforced at both propose and apply time. A prompt-injected proposal can only sit unapplied in a diff view, and can never target the foundation at all. |
